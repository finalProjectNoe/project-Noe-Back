...
describe('Noe', function () {
  this.timeout(0);
  const [owner, dev, user1, user2, veterinary1, vetApprouve1, superAdmin] = accounts;
  const NAME = 'Noe';
  const SYMBOL = 'NOE';
  ...
  it('Vétérinaire crée', async function () {
    ...
    const vet1 = await this.noe.getVeterinary(userVeterinary1);
  ...
  it('mints NFT to user by calling animalToken()', async function () {
    await this.noe.animalToken(user1, animal1, { from: owner });
